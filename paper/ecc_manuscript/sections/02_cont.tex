...
Here, $\mathfrak{A}$ is a linear operator $\mathcal{L}(X)$ acting on a Hilbert space $X: L^2[0,1] \times L^2[0,1]$ and $\underline{x}(\zeta,t)$, as defined previously, is the vector of functions describing the states of the system. The operator $\mathfrak{A}$ and its domain are defined in \eqref{eq:operator_A}. Also, $\mathfrak{B}$ is a linear operator that maps the scalar input from input-space onto the state space, as defined in \eqref{eq:operator_B}. Finally, the output operator $\mathfrak{C}$ is defined in \eqref{eq:operator_C}, where it maps the state to the output $y(t)$, which is the concentration at the reactor outlet according to \eqref{eq:BC}.
...
\begin{figure}[!htbp]
    \centering
    \includesvg[inkscapelatex=false, width=0.3\textwidth, keepaspectratio]{Figures/eig_val_dist_R_0.3.svg}
